...
{
    memcpy((unsigned char*)destination + ID_OFFSET, &(source->id), ID_SIZE);
    memcpy((unsigned char*)destination + USERNAME_OFFSET, &(source->username), USERNAME_SIZE);
    memcpy((unsigned char*)destination + EMAIL_OFFSET, &(source->email), EMAIL_SIZE);
}

void deserialize_row(void* source, Row* destination)
{
    memcpy(&(destination->id), (unsigned char*)source + ID_OFFSET, ID_SIZE);
    memcpy(&(destination->username), (unsigned char*)source + USERNAME_OFFSET, USERNAME_SIZE);
    memcpy(&(destination->email), (unsigned char*)source + EMAIL_OFFSET, EMAIL_SIZE);
}
...
void* cursor_value(Cursor* cursor)
{
    ...
    return (unsigned char*)page + byte_offset;
}
